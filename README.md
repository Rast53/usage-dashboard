...
- **Карточки провайдеров** — Z.AI (оба лимита: короткий 5ч + недельный), Command Code GOAT (кредиты + окна 5ч / неделя / месяц), Kimi Coding (недельная квота + окно 5ч), OpenCode Go (месячный лимит + окна 5ч / неделя, дата reset), DeepSeek (баланс), OpenRouter (кредиты). На каждой карточке расход **24ч и 7д** из snapshots. OpenRouter: разбивка по моделям из `/api/v1/activity`; остальные — «нет разбивки от провайдера». Цветовая подсветка: 🔴 критично / 🟡 внимание / 🟢 нормально
